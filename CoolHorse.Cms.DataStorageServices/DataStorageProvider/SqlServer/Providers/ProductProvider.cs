...
namespace CoolHorse.Cms.DataStorageServices.Providers.SqlServer
{
    ...
    public partial class SqlServerProvider : ProviderBase, IDataStorageProvider
    {
        ...
        {
            var script = string.Format("INSERT INTO Product(Title,Content,CategoryId) VALUES('{0}','{1}',{2});SELECT @@IDENTITY;",model.Title,model.Content,model.Category.Id);
        ...
        {
            var script = string.Format("UPDATE Product SET Title='{0}',Content = '{1}',CategoryId = {2} WHERE Id ={3}", model.Title, model.Content, model.Category.Id, model.Id);

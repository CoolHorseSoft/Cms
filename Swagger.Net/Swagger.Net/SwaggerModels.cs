...
namespace Swagger.Net
{
    public static class SwaggerGen
    {
        ...
        public static ResourceApi CreateResourceApi(ApiDescription api)
        {
            ...
            {
                path = "/" + "api/Values/" + api.ActionDescriptor.ActionName,
        ...
        private static string GetResponseClassFormattedXMLSchema(Type apiType)
        {
            var importer = new XmlReflectionImporter();
            var schemas = new XmlSchemas();
            var exporter = new XmlSchemaExporter(schemas);

            var xmlTypeMapping = importer.ImportTypeMapping(apiType);
            exporter.ExportTypeMapping(xmlTypeMapping);

            var responseClassSchemaWriter = new StringWriter();
            schemas[0].Write(responseClassSchemaWriter);

           return "<pre>" + HttpUtility.HtmlEncode(responseClassSchemaWriter.ToString()) + "</pre>";
        ...
        {
            var responseClassFormattedXMLSchema = GetResponseClassFormattedXMLSchema(api.ActionDescriptor.ReturnType);
            ...
            ResourceApiOperation rApiOperation = new ResourceApiOperation()
            {
                ...
                responseClass = responseClassFormattedXMLSchema,
        ...
        public static ResourceApiOperationParameter CreateResourceApiOperationParameter(ApiDescription api, ApiParameterDescription param, XmlCommentDocumentationProvider docProvider)
        {
            ...
            ResourceApiOperationParameter parameter = new ResourceApiOperationParameter()
            {
                ...
                name = param.Name,
                description = param.Documentation + docProvider.GetDefaultValueDescriptionIfAny(param.ParameterDescriptor),
